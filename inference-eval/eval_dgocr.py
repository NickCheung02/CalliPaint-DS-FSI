...
def parse_args():
    ...
    parser.add_argument(
        "--img_dir",
        type=str,
        default='/data/vdb/yuxiang.tyx/AIGC/eval/img_folder',
...
if USING_DLC:
    img_dir = img_dir.replace('/data/vdb', '/mnt/data', 1)
    input_json = input_json.replace('/data/vdb', '/mnt/data', 1)
...
def main():
    ...
    for i in tqdm(range(len(data_list)), desc='evaluate'):
        ...
        if n_lines > 0:
            ...
            preds_all = []
            rst_all = predictor(pred_texts, batch_size=8)
            preds_all += [rst['text'] for rst in rst_all]
            if PRINT_DEBUG:
                for idx, pt in enumerate(pred_texts):
                    cv2.imwrite(f'{i}_{idx}.jpg', pt)
