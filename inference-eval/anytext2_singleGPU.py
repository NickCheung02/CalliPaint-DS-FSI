...
max_chars = 20
max_lines = 20
font = ImageFont.truetype('/home/610-zzy/Dataset/font/huangdengrong.TTF', size=60)
glyph_scale = 1
use_fp16 = True
...
if __name__ == '__main__':
   ...
   # 确保这一行和上面的 for 循环是对齐的（通常是4个空格）
    if len(times) > 1:
        times = times[1:]  # 忽略第一次加载的预热耗时
        mean_batch_time_ms = np.mean(times) # 平均每个 batch 的耗时 (ms)
        mean_single_img_time_ms = mean_batch_time_ms / num_samples # 除以 batch 大小，得单张耗时
        
        print(f'平均 Batch 耗时: {mean_batch_time_ms:.2f} ms')
        print(f'单张图片平均耗时: {mean_single_img_time_ms:.2f} ms (Batch Size: {num_samples})')
        
    elif len(times) == 1:
        batch_time_ms = times[0]
        single_img_time_ms = batch_time_ms / num_samples
        
        print(f'Batch 耗时: {batch_time_ms:.2f} ms')
        print(f'单张图片耗时: {single_img_time_ms:.2f} ms (Batch Size: {num_samples})')
